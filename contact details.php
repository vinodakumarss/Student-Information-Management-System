...
function get_records($offset, $limit){
    ...
    $fetch_rec = $USER_FUN->show_rec('contact', $offset, $limit);
    if($fetch_rec){
        foreach($fetch_rec as $rec_data){
            $tbl_structure .= '<tr class="v-set"><th scope="row">'.$rec_data['id'].'</th><td scope="row">'.$rec_data['username'].'</td><td scope="row">'.$rec_data['email'].'</td><td scope="row">'.$rec_data['message'].'</td></tr>';
        }
    }
    else{
...
function pagination(){
    ...
    $per_page_records = 7;
    $total_records = $USER_FUN->rec_count('contact');
    ...
    if($page_number < $total_buttons && ($last_page == $total_buttons || $last_page > $total_buttons)){
        ...
        if($page_number >= 2){
            $pagination .= '<li class="page-item"><a class="page-link" href="contact details.php?page=1">&lt;&lt;</a></li>';
            $pagination .= '<li class="page-item"><a class="page-link" href="contact details.php?page='.($page_number - 1).'">Previous</a></li>';
        }
        ...
        if($page_number >= ($half + 2)){ 
            ...
            for($j=($page_number-$half); $j<=($page_number+$half); $j++){
                ...
                if($j == $page_number){
                    ...
                    $pagination .= '<li class="page-item"><a class="page-link" href="contact details.php?page='.$j.'">'.$j.'</a></li>';
                }
        ...
        $pagination .= '<li class="page-item"><a class="page-link" href="contact details.php?page='.($page_number + 1).'">Next</a></li>';
        $pagination .= '<li class="page-item"><a class="page-link" href="contact details.php?page='.$last_page.'">&gt;&gt;</a></li>';
    }
    ...
    <div class="container-fluid">
        ...
        <div class="container">
            <ul class="nav justify-content-center bg-success">
                <li class="nav-item">
                    <div class="nav-link heading">Contact Details</div>
                </li>
            </ul>
        </div>
